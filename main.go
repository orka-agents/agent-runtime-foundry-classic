...
import (
	...
	"github.com/orka-agents/agent-runtime-foundry-classic/internal/sanitize"
)
...
const (
	...
	maxFoundryBrokeredFrameBytes = 4 << 20
	foundryCancelTimeout         = 30 * time.Second
	threadCleanupRetry           = time.Minute
...
func main() {
	cfg := loadConfig()
	tokenProvider, err := newAzureFoundryTokenProvider()
	if err != nil {
		log.Fatal(err)
	}
	s := &server{
		cfg:                cfg,
		client:             newFoundryHTTPClient(cfg.endpoint),
	...
	mux := s.handler()
	log.Printf("Foundry classic AgentRuntime adapter listening on %s (runtime=%s endpoint=%s)", cfg.addr, cfg.runtimeName, sanitizeEndpoint(cfg.endpoint)) //nolint:lll
	server := &http.Server{
		Addr:              cfg.addr,
		Handler:           mux,
...
func loadConfig() config {
	return config{
		...
		runtimeName:   firstNonBlank(os.Getenv(envRuntimeName), "foundry-classic-runtime"),
...
func (s *server) health(w http.ResponseWriter, r *http.Request) {
	...
	msg := "ready"
	if ready {
		credentialCtx, cancel := context.WithTimeout(r.Context(), 5*time.Second)
		_, err := s.tokenProvider.AccessToken(credentialCtx)
		cancel()
		if err != nil {
			ready = false
			status = harness.HealthStatusDegraded
			msg = "Azure credential unavailable"
		}
	} else {
		status = harness.HealthStatusDegraded
		msg = "adapter bearer, safe Foundry endpoint, agent id, and Azure credential are required; " +
			foundryEndpointRequirement
	}
	harness.WriteJSON(w, http.StatusOK, harness.HealthResponse{Version: harness.ProtocolVersion, Status: status, Ready: ready, CheckedAt: time.Now().UTC(), Message: msg, Metadata: map[string]string{"backend": "foundry-classic"}}) //nolint:lll
}
...
func (s *server) capabilities(w http.ResponseWriter, r *http.Request) {
	...
	harness.WriteJSON(w, http.StatusOK, harness.CapabilitiesResponse{
		...
		RuntimeName: s.cfg.runtimeName, RuntimeVersion: "foundry-classic-adapter", ProviderKind: harness.ProviderKindRemote,
		...
		Metadata: map[string]string{"backend": "foundry-classic"},
	})
}
...
func (s *server) startTurn(w http.ResponseWriter, r *http.Request) {
	...
	req = prepareFoundryStartRequest(req)
	...
	s.mu.Lock()
	s.pruneTurnTombstonesLocked(time.Now().UTC())
	if tombstone, exists := s.turnTombstones[req.TurnID]; exists {
		s.mu.Unlock()
		if sameStartTurnRequest(tombstone.request, req) {
			harness.WriteError(w, http.StatusConflict, "turn already completed and is no longer replayable")
		} else {
			harness.WriteError(w, http.StatusConflict, "turn id was already consumed")
		}
		return
	}
	...
	turn := &turnState{
		request:         req,
		pollDeadline:    foundryTurnDeadline(time.Now(), req.Deadline, s.cfg.pollTimeout),
		initializing:    true,
		pendingTools:    map[string]string{},
		bufferedResults: map[string]harness.ToolCallResult{},
		emittedResults:  map[string]struct{}{},
	...
	s.mu.Unlock()
	backendCtx, cancel := context.WithDeadline(context.Background(), turn.pollDeadline)
	defer cancel()
...
func (s *server) continueTurn(w http.ResponseWriter, r *http.Request, turn *turnState) {
	...
	s.mu.Unlock()
	if err := s.validateBrokeredResultFrames(turn, req.ToolResults); err != nil {
...
func (s *server) cancelTurn(w http.ResponseWriter, r *http.Request, turn *turnState) {
	...
	if !cancelRequestMatchesTurn(req, turn.request) {
...
func (s *server) pollFoundry(ctx context.Context, turn *turnState) error {
	...
	deadline := s.foundryPollDeadline(turn)
	...
	s.expireFoundryTurn(turn)
	return nil
}

func (s *server) foundryPollDeadline(turn *turnState) time.Time {
	if !turn.pollDeadline.IsZero() {
		return turn.pollDeadline
	}
	return foundryTurnDeadline(time.Now(), turn.request.Deadline, s.cfg.pollTimeout)
}
...
func (s *server) cancelFoundryRun(ctx context.Context, turn *turnState) error {
	s.mu.Lock()
	...
	path := fmt.Sprintf("/threads/%s/runs/%s", url.PathEscape(threadID), url.PathEscape(runID))
	var run foundryRun
	if err := s.doJSON(ctx, http.MethodPost, path+"/cancel", nil, &run); err != nil {
		return err
	}
	return s.waitFoundryRunCancelled(ctx, path, run.Status)
}

func (s *server) waitFoundryRunCancelled(ctx context.Context, runPath, status string) error {
	for {
		switch status {
		case "cancelled":
			return nil
		case "completed", "failed", "expired", "incomplete":
			return fmt.Errorf("foundry run reached terminal status %q before cancellation completed", status)
		}
		select {
		case <-ctx.Done():
			return ctx.Err()
		...
		var run foundryRun
		if err := s.doJSON(ctx, http.MethodGet, runPath, nil, &run); err != nil {
			return err
		}
		status = run.Status
	}
...
func (s *server) cancelAndFailFoundryTurn(
...
func (s *server) expireFoundryTurn(turn *turnState) {
	ctx, cancel := context.WithTimeout(context.Background(), foundryCancelTimeout)
	defer cancel()
...
func (s *server) validateBrokeredResultFrames(
	turn *turnState,
	...
	for _, result := range results {
		...
		if err != nil {
			return fmt.Errorf("brokered tool result %q cannot be encoded: %w", result.ToolCallID, err)
		}
		if len(encoded) > maxFoundryBrokeredFrameBytes {
			return fmt.Errorf(
				"brokered tool result %q frame exceeds adapter limit of %d bytes",
				result.ToolCallID,
				maxFoundryBrokeredFrameBytes,
			)
		}
	}
	return nil
...
func (s *server) scheduleTurnCleanupLocked(turn *turnState) {
	...
	time.AfterFunc(defaultStateRetention, func() {
		now := time.Now().UTC()
		s.mu.Lock()
		s.recordTurnTombstoneLocked(turn, now)
		delete(s.turns, turnID)
		s.mu.Unlock()
		s.cleanupExpiredFoundryThreads(now)
	})
}

type expiredFoundryThread struct {
	sessionID harness.RuntimeSessionID
	threadID  string
	seenAt    time.Time
}

func (s *server) cleanupExpiredFoundryThreads(now time.Time) {
	s.sessionMu.Lock()
	defer s.sessionMu.Unlock()
	cutoff := now.Add(-defaultStateRetention)
	s.mu.Lock()
	candidates := make([]expiredFoundryThread, 0)
	for sessionID, seen := range s.runtimeThreadSeen {
		threadID := s.runtimeThreads[sessionID]
		if seen.Before(cutoff) && threadID != "" && !s.foundryThreadInUseLocked(threadID) {
			candidates = append(candidates, expiredFoundryThread{
				sessionID: sessionID,
				threadID:  threadID,
				seenAt:    seen,
			})
	...
	for _, candidate := range candidates {
		ctx, cancel := context.WithTimeout(context.Background(), foundryCancelTimeout)
		err := s.doJSON(ctx, http.MethodDelete, "/threads/"+url.PathEscape(candidate.threadID), nil, nil)
		cancel()
		if err != nil {
			retry = true
		...
		s.mu.Lock()
		if s.runtimeThreads[candidate.sessionID] == candidate.threadID &&
			s.runtimeThreadSeen[candidate.sessionID].Equal(candidate.seenAt) {
			delete(s.runtimeThreads, candidate.sessionID)
			delete(s.runtimeThreadSeen, candidate.sessionID)
		}
		s.mu.Unlock()
	}
	if retry {
		time.AfterFunc(threadCleanupRetry, func() {
			s.cleanupExpiredFoundryThreads(time.Now().UTC())
		})
	}
}

func (s *server) foundryThreadInUseLocked(threadID string) bool {
	for _, turn := range s.turns {
		if !turn.completed && turn.threadID == threadID {
			return true
		}
	}
	return false
...
func (s *server) recordTurnTombstoneLocked(turn *turnState, now time.Time) {
	if s.turnTombstones == nil {
		s.turnTombstones = make(map[harness.HarnessTurnID]turnTombstone)
	}
	s.pruneTurnTombstonesLocked(now)
	if len(s.turnTombstones) >= maxTurnTombstones {
		var oldestID harness.HarnessTurnID
		var oldestExpiry time.Time
		for turnID, tombstone := range s.turnTombstones {
			if oldestExpiry.IsZero() || tombstone.expiresAt.Before(oldestExpiry) {
				oldestID = turnID
				oldestExpiry = tombstone.expiresAt
			}
		}
		delete(s.turnTombstones, oldestID)
	}
	s.turnTombstones[turn.request.TurnID] = turnTombstone{
		request:   turn.request,
		expiresAt: now.Add(defaultTombstoneRetention),
	}
}

func (s *server) pruneTurnTombstonesLocked(now time.Time) {
	for turnID, tombstone := range s.turnTombstones {
		if !tombstone.expiresAt.After(now) {
			delete(s.turnTombstones, turnID)
		}
	}
}
...
func (s *server) appendFrameLocked(turn *turnState, typ harness.FrameType, summary string, mutate func(*harness.HarnessEventFrame)) { //nolint:lll
	seq := int64(len(turn.frames) + 1)
	frame := harness.HarnessEventFrame{Version: harness.ProtocolVersion, Type: typ, RuntimeSessionID: turn.request.RuntimeSessionID, TurnID: turn.request.TurnID, CorrelationID: turn.request.CorrelationID, Seq: seq, CreatedAt: time.Now().UTC(), Summary: summary, Metadata: map[string]string{"backend": "foundry-classic"}} //nolint:lll
	if mutate != nil {
		mutate(&frame)
	}
